...
if reuse_trained_model:
    model = tp.DMRModel.load('./models/papers_topic_num_10.model')
else:
    # 데이터 로드 및 전처리
    timestamps, dataset = papers_data.load_for_dmr(timestamp_index=1, target_index=3)

    # DMR 모델 학습 및 저장
    model = dmr_model(dataset, timestamps, topic_number)
    model.save('./models/papers.model', True)


# document별 dominant topic 정보 저장
topic_model = pyTextMinerTopicModel()
df_topic_sents_keywords, matrix = topic_model.format_topics_sentences(topic_number=topic_number, mdl=model)
# formatting
...
df_dominant_topic_for_each_doc.to_csv('./results/papers_dominent_topic_for_each_doc.csv', index=False,
                                      encoding='utf-8-sig')
...
df_topic_label_keyword.to_csv('./results/papers_topic_label_keyword.csv', index=False, encoding='utf-8-sig')


# timestamp별 topic score 계산 및 저장
df_topic_score = topic_scoring(model)
print(df_topic_score)
df_topic_score.to_csv('./results/papers_topic_score.csv', encoding='utf-8-sig')


# timestamp별 topic score line graph
df_topic_score.T.plot(style='.-', grid=True)
plt.title('Paper Topic Score')
ylim = max(abs(min(df_topic_score.min())), abs(max(df_topic_score.max()))) + 0.5
plt.ylim(-ylim, ylim)
plt.legend(loc='lower right', fontsize=8)
plt.savefig('./results/papers_topic_score.png')
plt.show()
...
plt.title('Paper Topic Distributions')
plt.legend(loc='lower right', fontsize=8)
plt.savefig('./results/papers_topic_dist.png')
plt.show()