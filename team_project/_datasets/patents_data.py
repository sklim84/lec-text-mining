...
####################
# USPTO patents
# - Format : publication number | publication date(yyyy-mm-dd) | title | abstract
# - 기간 : 2008.01-2022.04
####################

def load_for_coword(target_index):
    here = pathlib.Path(__file__).resolve().parent
    loc_data = here / 'patents.xlsx'
    loc_stopwords = here / 'stopwordsEng.txt'

    # 데이터 로드
    df_patents = pd.read_excel(loc_data)
    target = df_patents.iloc[:, [target_index]].astype(str).values.tolist()
    # [[document1], ...] → [document1, ...]
    target = sum(target, [])
...
def load_for_dmr(timestamp_index, target_index, timestamp_pattern='%Y', reuse_preproc=False):
    ...
    # 기전처리된 파일 사용 시
    if reuse_preproc:
        with open(here / 'patents_pp_for_dmr.pkl', 'rb') as fin:
    ...
    # 데이터 로드
    df_patents = pd.read_excel(loc_data)
    timestamps = df_patents.iloc[:, timestamp_index].dt.strftime(timestamp_pattern).tolist()
    target = df_patents.iloc[:, [target_index]].astype(str).values.tolist()
    # [[document1], ...] → [document1, ...]
    target = sum(target, [])
    ...
    # 전처리된 결과 저장
    with open(here / 'patents_pp_for_dmr.pkl', 'wb') as fout:
...
def load_for_burstiness(timestamp_index, target_index, timestamp_pattern='%Y'):
    ...
    # timestamp data format 변경
    df_patents.iloc[:, timestamp_index] = df_patents.iloc[:, timestamp_index].dt.strftime(timestamp_pattern)
    ...
    pipeline = ptm.Pipeline(ptm.splitter.NLTK(),
                            ...
                            ptm.ngram.NGramTokenizer(1, 1),
                            ptm.helper.StopwordFilter(file=loc_stopwords))
    result = pipeline.processCorpus(df_patents.iloc[:, target_index])
    ...
    # 전처리 결과 반영
    df_patents.iloc[:, target_index] = documents
    return df_patents
...
def load_for_bert(timestamp_index, target_index, timestamp_pattern='%Y', reuse_preproc=False):
    ...
    # 기전처리된 파일 사용 시
    if reuse_preproc:
        with open(here / 'patents_pp_for_bert.pkl', 'rb') as fin:
            timestamps = pickle.load(fin)
            documents = pickle.load(fin)
        fin.close()
        return timestamps, documents
    ...
    # 전처리된 결과 저장
    with open(here / 'patents_pp_for_bert.pkl', 'wb') as fout:
        pickle.dump(timestamps, fout)
        pickle.dump(documents, fout)
    fout.close()
