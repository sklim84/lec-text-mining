...
def compute_term_burstiness(dataset):
    ...
    for key, item in grouped_df:
        ...
        print('time_stamp : {}'.format(str(time_stamp)), len(vocab), time.time() - t0)
    ...
    print('##### vocabulary 크기 : ' + str(len(vocabulary.keys())))
    ...
    print('##### 연도별 Document-term matrix 생성 시작')
    for key, item in grouped_df:
        a_group = grouped_df.get_group(key)
        ...
        time_stamp = a_group.iloc[:, 0].tolist()[0]
        print('time_stamp : {}'.format(str(time_stamp)), time.time() - t0)

    print('##### Document-term matrix 수 : ' + str(len(vectors)))

    # Summing the vectors
    # We sum the vectors along columns, so that we have the popularity of each term in each year.

    print('##### Summing the vectors 시작')
    # (docs, terms) -> (1, terms)
    summed_vectors = []
    for y in range(len(vectors)):
        vector = vectors[y]
        print('orignral vector shape : {}'.format(str(vector.shape)))
        ...
        print('result vector shape : {}'.format(str(np.sum(vector, axis=0).shape)))
    ...
    print('stacked vectors matrix : {}'.format(stacked_vectors.shape))
    ...
    print('##### signification 계산 결과 matrix : {}'.format(stacked_vectors.shape))

    # Calculate burstiness

    long_ma, short_ma, significance_ma, macd, signal, hist = calc_macd(stacked_vectors)
    print('##### MACD 계산 결과 : {}'.format(str(macd)))
    scaling_factor = calc_scaling(significance_ma, "sqrt")
    print('##### significance_ma 계산 결과 : {}'.format(str(macd)))
    burstiness_over_time = calc_burstiness(hist, scaling_factor)
    burstiness = max_burstiness(burstiness_over_time)
    ...
    print('##### bursts 10 samples : {}'.format(str(bursts[:10])))
...
dataset = '../homework/assignment2/data/news_articles_201701_201812.csv'
